...
// Img url to use in changImg function later
  imgUrl = '';
...
countriesCode = {
  ...
  // Add New Contact
  addContact(form) {
    // Get complete mobile number with city code
    form.value.mobileNumber = form.value.cityCode + form.value.mobileNumber;

    // Implement addNewContact method from Contacts service
    const data: Contacts = form.value;
    this.contacts.addNewContact(data.userId, data.email, data.userName, data.image, data.mobileNumber, data.firstName, data.lastName);
    this.router.navigate(['/']);

  }
  // Toggle change img section
  chooseFile() {
    this.show = !this.show;
  }
